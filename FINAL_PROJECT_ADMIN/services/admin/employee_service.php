<?php
    ...
    function get_Employees(){
        ...
        $sql = "SELECT users.id, users.name, users.email, users.profile_picture, employee_details.salary,employee_details.role,employee_details.bonus,employee_details.rating FROM users INNER JOIN employee_details ON users.id = employee_details.user_id where users.userType='Employee'";
        $result = mysqli_query($conn, $sql);
    ...
    function get_empDataById($id){
        if($id != null || $id !=""){
            ...
            $sql = "SELECT users.id, users.name, users.email, users.profile_picture, employee_details.salary,employee_details.role,employee_details.bonus,employee_details.rating FROM users INNER JOIN employee_details ON users.id = employee_details.user_id where users.id='$id'";
            $result = mysqli_query($conn, $sql);
    ...
    function updateEmp($empData){
        ...
        $sqlEmp = "update employee_details set role='$role', salary='$salary', bonus='$bonus', rating='$rating' where user_id='$user_id'";
    ...
    function insertEmp($empDataAdd){
        ...
        if($status){
            ...
            $sqlEmp = "insert into employee_details (id,role,salary,bonus,rating,user_id) values('','$empDataAdd->role','$empDataAdd->salary','0','0','$id')";
